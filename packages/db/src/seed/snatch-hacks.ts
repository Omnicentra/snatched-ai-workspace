...
import { snatchHacks } from "../schema";
import { eq } from "drizzle-orm";

export const snatchHacksData = [
  {
    id: 1,
    title: "Posture Check",
...
export async function seedSnatchHacks() {
  console.log("🌱 Seeding snatch hacks...");

  for (const hack of snatchHacksData) {
    // Check if hack already exists by title
    const existingHack = await db.query.snatchHacks.findFirst({
      where: eq(snatchHacks.title, hack.title),
    });

    // Skip if already exists
    if (existingHack) {
      console.log(`⚠️ Snatch hack "${hack.title}" already exists, skipping`);
      continue;
    }

    // Insert new hack
    await db.insert(snatchHacks).values({
      title: hack.title,
      description: hack.description,
      icon: hack.icon,
      color: hack.color,
      bgColor: hack.bgColor,
      benefits: hack.benefits,
      instructions: hack.instructions,
    });
  }

  console.log("✅ Snatch hacks seeded!");
}
